...
|---|---|---|---|---|
| `drop-window.yml` | 매일 `23:00, 23:30, 00:00, 00:15, 00:35, 00:40, 00:45, 00:50, 00:55, 01:00, 01:05` + 선택적 외부 API 트리거 | 매일 `08:00~10:05` 백업 트리거 후 실제 폴링은 `09:50~10:10`에만 수행 | 최대 20분 폴링 | 1분 |
...
> GitHub Actions `schedule`은 부하 시 지연되거나 드롭될 수 있습니다. 이 저장소는 08:00 KST부터 백업 cron을 두고 윈도우 시작 전에는 대기하도록 바꿨지만, `09:50 정각 시작`을 강하게 원하면 외부 스케줄러가 GitHub API로 `repository_dispatch`를 호출하는 구성이 가장 정확합니다.
...
| 09:50에 시작 안 함 | GitHub cron은 부하 시 지연/드롭 가능. 현재는 08:00 KST부터 여러 백업 cron + 윈도우 대기 로직이 들어가 있음. 더 정확히 맞추려면 외부 스케줄러에서 `repository_dispatch` 호출 |
