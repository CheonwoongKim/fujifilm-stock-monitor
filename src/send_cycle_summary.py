...
def load_summary_context() -> SummaryContext:
...
def build_stock_line(*, transitions_detected: int, saw_any_in_stock: bool) -> str:
    if transitions_detected > 0:
        return "윈도우 중 재고 변동을 감지했습니다."
    if saw_any_in_stock:
        return "재고가 입고 상태로 확인되었지만, 이번 윈도우에서 새 전이는 감지되지 않았습니다."
    return "10:10 KST까지 재고가 확인되지 않았습니다."


def build_summary_message(cycle_state: dict) -> str:
    checks = int(cycle_state.get("checks", 0))
    transitions_detected = int(
        cycle_state.get("transitions_detected", cycle_state.get("alerts_sent", 0))
    )
    ...
    return (
        "✅ <b>오늘 재고 모니터링이 정상 완료되었습니다</b>\n"
        f"{build_stock_line(transitions_detected=transitions_detected, saw_any_in_stock=saw_any_in_stock)}\n\n"
        f"폴링 횟수: <b>{checks}</b>\n"
        f"입고 전이 감지 횟수: <b>{transitions_detected}</b>\n"
...
def persist_summary_marker(state_path: Path, persisted_state: dict, *, cycle_key: str, timezone_name: str) -> None:
...
def main() -> int:
    ...
    cycle_state = load_json(context.cycle_state_path)
    persisted_state = load_json(context.state_path)
    ...
    if context.skipped:
        text = (
            ...
            f"{context.window_label} 구간이 지난 뒤 워크플로우가 시작되어 오늘 폴링은 건너뛰었습니다.\n"
            f"사유: <code>{context.cycle_end_reason}</code>"
        )
        notify.send(text, product_url=context.product_url)
    elif checks == 0:
        text = (
            "⚠️ <b>재고 모니터링이 정상 완료되지 않았습니다</b>\n"
            f"{context.window_label} 동안 실행 기록이 없습니다.\n"
            f"사유: <code>{context.cycle_end_reason}</code>"
        )
        notify.send(text, product_url=context.product_url)
    else:
        notify.send(
            build_summary_message(cycle_state),
            product_url=context.product_url,
        )

    persist_summary_marker(
        context.state_path,
        persisted_state,
        cycle_key=cycle_key,
        timezone_name=context.timezone_name,
